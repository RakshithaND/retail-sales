# RetailX
Smart Sales Monitor
A full-stack analytics dashboard built using:

- **React (Vite)** — Frontend
- **Node.js + Express** — Backend
- **CSV-based dataset** — Sales data

## 🚀 Features

### 📊 Sales Dashboard
- Search and filtering  
- Category, gender, and age filters  
- Discount calculations  
- Clean analytics cards  
- Transaction table  

### 📈 Nexus Analytics
- Gender distribution  
- Category-wise analytics  
- Interactive charts  

### 📥 Intake System
- Customer & region filtering  
- Upcoming enhancements  

### 🧾 Invoices (Placeholder)
- Proforma Invoices *(Yet to add)*  
- Final Invoices *(Yet to add)*  

---

## ▶️ How to Run the Project

### **Backend**
cd backend
npm install
node src/index.js

### **Frontend**
cd frontend
npm install
node run dev

### **Snapshots**

<img width="1879" height="993" alt="Screenshot 2025-12-08 042318" src="https://github.com/user-attachments/assets/5a3e72f9-16d7-434d-8fa4-be1679a51521" />
<img width="1883" height="989" alt="image" src="https://github.com/user-attachments/assets/254e930b-bb4d-4028-ad27-8db5b1b0fa6b" />

